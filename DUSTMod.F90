...
!-----------------------------------------------------------------------------------------------------
! MODIFICATIONS TO MODULE ADDED IN LINES 31-40, 162-176, 211-252, 259-263 
!-----------------------------------------------------------------------------------------------------
...
!-Variables I Added -------------------------------------------------------------------------------------
    ...
    real(r8), pointer :: tlai_dust(:)     ! FOR DUSTMOD one-sided leaf area index, no burying by snow
    real(r8), pointer :: tsai_dust(:)     ! FOR DUSTMOD one-sided stem area index, no burying by snow
 
    type(file_desc_t)  :: ncid    ! netcdf id
    character(len=256) :: locfn    ! local file name Jan
    
    integer :: year  ! year (0,...) for nstep
    integer :: month ! month (1, ..., 12) for nstep
    integer :: day   ! day of month (1, ..., 31) for nstep
    integer :: secs  ! seconds into current date for nstepi
...
! PULL TLAI_DUST FROM RESTART FILES

    tlai_dust => pps%tlai_dust
    tsai_dust => pps%tsai_dust

    call get_curr_date(year,month,day,secs)

    if (month == 1) then
     call getfil("[INSERT JAN CASE1 RESTART FILE WITH ADDED TLAI_DUST/TSAI_DUST VARIABLES]",locfn,0)
    else if (month == 2) then
     call getfil("[INSERT FEB CASE1 RESTART FILE WITH ADDED TLAI_DUST/TSAI_DUST VARIABLES]",locfn,0)
    else if (month == 3) then
     call getfil("[INSERT MAR CASE1 RESTART FILE WITH ADDED TLAI_DUST/TSAI_DUST VARIABLES]",locfn,0)
    else if (month == 4) then
     call getfil("[INSERT APR CASE1 RESTART FILE WITH ADDED TLAI_DUST/TSAI_DUST VARIABLES]",locfn,0)
    else if (month == 5) then
     call getfil("[INSERT MAY CASE1 RESTART FILE WITH ADDED TLAI_DUST/TSAI_DUST VARIABLES]",locfn,0)
    else if (month == 6) then
     call getfil("[INSERT JUN CASE1 RESTART FILE WITH ADDED TLAI_DUST/TSAI_DUST VARIABLES]",locfn,0)
    else if (month == 7) then
     call getfil("[INSERT JUL CASE1 RESTART FILE WITH ADDED TLAI_DUST/TSAI_DUST VARIABLES]",locfn,0)
    else if (month == 8) then
     call getfil("[INSERT AUG CASE1 RESTART FILE WITH ADDED TLAI_DUST/TSAI_DUST VARIABLES]",locfn,0)
    else if (month == 9) then
     call getfil("[INSERT SEP CASE1 RESTART FILE WITH ADDED TLAI_DUST/TSAI_DUST VARIABLES]",locfn,0)
    else if (month == 10) then
     call getfil("[INSERT OCT CASE1 RESTART FILE WITH ADDED TLAI_DUST/TSAI_DUST VARIABLES]",locfn,0)
    else if (month == 11) then
     call getfil("[INSERT NOV CASE1 RESTART FILE WITH ADDED TLAI_DUST/TSAI_DUST VARIABLES]",locfn,0)
    else if (month == 12) then
     call getfil("[INSERT DEC CASE1 RESTART FILE WITH ADDED TLAI_DUST/TSAI_DUST VARIABLES]",locfn,0)
    end if

    call ncd_pio_openfile(ncid,locfn,0)
    call ncd_io(ncid=ncid, varname='tlai_dust', flag='read', data=tlai_dust, dim1name=grlnd)
    call ncd_io(ncid=ncid, varname='tsai_dust', flag='read', data=tsai_dust, dim1name=grlnd)
...
! Comment out the line above and add the line below...
       ...
       ttlai(p) = tlai_dust(p)+tsai_dust(p) 
!-------------------------------------------------------------------------------------------------------------------
    enddo
...
if (day == 2) then
  write(iulog,*)'flx_mss_vrt_dust = dustflxt',size(flx_mss_vrt_dst)
end if
